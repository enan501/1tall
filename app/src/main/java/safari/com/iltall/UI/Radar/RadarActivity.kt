    ...
    private fun addMarkers() {
        for(marker in markerPoint){
            if(dist(curLocation, marker.location)<100){
                ...
                var btn = Button(this)
                var x = 111*(marker.location.latitude - curLocation.latitude)
                var y = (Math.cos(marker.location.latitude)*6400*2*3.14/360)*(marker.location.longitude - curLocation.longitude)
                var layoutParams = FrameLayout.LayoutParams(80,80)
                btn.setBackgroundResource(R.drawable.marker)
                Log.v("ss",wc.toString()+" / "+hc)
                Log.v("ss",(wc + 200 * x).toInt().toString()+" / "+(hc + 6.9 * y).toInt())
                layoutParams.leftMargin = (wc + 600 * x).toInt()
                layoutParams.topMargin = (hc + 444 * y).toInt()
...
//                    wmlp.y = 200   //y position
    ...
    fun dist(cur: MyLocation, marker:MyLocation):Double{
        var x = (Math.cos(marker!!.latitude)*6400*2*3.14/360)*Math.abs(marker!!.longitude-cur!!.longitude)
    ...
    fun drawMyLoc(){
        ...
        Glide.with(this).load(safari.com.iltall.R.raw.working).override(50,50).diskCacheStrategy(
            DiskCacheStrategy.SOURCE).into(gifimg)
    }
